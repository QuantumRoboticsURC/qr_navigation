...
class CenterAndApproach():
    def __init__(self):
        ...
        self.kp_distance_error = PlatfromConstants.CENTER_AND_APPROACH_LINEAR_KP    # comes from  1[rad/s] = +-5 [max error meters] * kpde
        # ___ error initialization ___
        self.angle_error = 0.0
        self.distance_error = PlatfromConstants.CENTER_AND_APPROACH_LINEAR_SET_POINT + PlatfromConstants.CENTER_AND_APPROACH_LINEAR_ERROR_TRESHOLD + 0.01        
        # ___ vel command ___
    ...
    def calculate_error(self):
        ...
        self.distance_error = self.aruco_position.x - PlatfromConstants.CENTER_AND_APPROACH_LINEAR_SET_POINT                
    ...
    def main(self):
        while not rospy.is_shutdown():
            if self.started:
                ...
                if abs(self.angle_error) > PlatfromConstants.CENTER_AND_APPROACH_ANGULAR_ERROR_TRESHOLD:
                    ...
                    self.command_velocity.angular.z = nav_functions.saturate_signal(candidate_angular_vel * self.wheel_overshot_softener, PlatfromConstants.CENTER_AND_APPROACH_ANGULAR_SATURATION_VAL)
                    self.prev_angular_velocity = candidate_angular_vel                                        
                elif self.distance_error > PlatfromConstants.CENTER_AND_APPROACH_LINEAR_ERROR_TRESHOLD:
                    candidate_linear_vel = self.distance_error * self.kp_distance_error
                    self.command_velocity.linear.x = nav_functions.saturate_signal(candidate_linear_vel, PlatfromConstants.CENTER_AND_APPROACH_LINEAR_SATURATION_VAL)                                        
